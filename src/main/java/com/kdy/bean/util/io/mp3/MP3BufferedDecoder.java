package com.kdy.bean.util.io.mp3;
...
public class MP3BufferedDecoder {
	...
	public static int frequency(MP3HeaderData headerData) {
		return MP3HeaderData.frequencies[headerData.h_version][headerData.h_sample_frequency];
	}
	...
	public static int samples_per_frame(MP3HeaderData headerData) {
		...
		try {
			if (headerData.h_vbr == true) {
				...
				if ((headerData.h_version == MP3HeaderData.MPEG2_LSF) || (headerData.h_version == MP3HeaderData.MPEG25_LSF)) {
					ret /= 2;
				}
			} else {
				// {{22050, 24000, 16000},
				...
				int samples_per_frame_array[][][] = {
					...
					//{26.12245f, 24.0f, 36.0f}
					{{192, 192, 192}, {576, 576, 576}, {576, 576, 576}},

					// {44100, 48000, 32000},
					//{8.707483f,  8.0f, 12.0f}
					//{26.12245f, 24.0f, 36.0f}
					//{26.12245f, 24.0f, 36.0f}
					{{384, 384, 384}, {1152, 1152, 1152}, {1152, 1152, 1152}},

					// {11025, 12000, 8000}};	// SZD: MPEG25
					//{8.707483f,  8.0f, 12.0f}
					//{26.12245f, 24.0f, 36.0f}
					//{26.12245f, 24.0f, 36.0f}
					{{96, 96, 96}, {288, 288, 288}, {288, 288, 288}}};
	...
	public static boolean isSyncMark(int headerstring, int syncmode, MP3HeaderData headerData) {
		...
		if (syncmode == MP3HeaderData.INITIAL_SYNC) {
			...
			sync = ((headerstring & 0xFFF80C00) == word) && (((headerstring & 0x000000C0) == 0x000000C0) == headerData.single_ch_mode);
		}
	...
	public static int calculateFrameSize(MP3HeaderData headerData) {
		int framesize = 0;
		@SuppressWarnings("unused") int nSlots;

		if (headerData.h_layer == 1) {
			framesize = (12 * MP3HeaderData.bitrates[headerData.h_version][0][headerData.h_bitrate_index]) / MP3HeaderData.frequencies[headerData.h_version][headerData.h_sample_frequency];
			if (headerData.h_padding_bit != 0) {
				framesize++;
			}
			framesize <<= 2;        // one slot is 4 bytes long
			nSlots = 0;
		} else {
			framesize = (144 * MP3HeaderData.bitrates[headerData.h_version][headerData.h_layer - 1][headerData.h_bitrate_index]) / MP3HeaderData.frequencies[headerData.h_version][headerData.h_sample_frequency];
			if (headerData.h_version == MP3HeaderData.MPEG2_LSF || headerData.h_version == MP3HeaderData.MPEG25_LSF) {
				framesize >>= 1;    // SZD
			}
			if (headerData.h_padding_bit != 0) {
