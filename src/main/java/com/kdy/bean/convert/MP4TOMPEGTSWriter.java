...
import static com.kdy.bean.util.io.ts.TransportStreamUtils.SYNCBYTE;
import static com.kdy.bean.util.io.ts.TransportStreamUtils.TIME_SCALE;
import static com.kdy.bean.util.io.ts.TransportStreamUtils.TS_PACKETLEN;
import static com.kdy.bean.util.io.ts.TransportStreamUtils.audioCodecToStreamType;
import static com.kdy.bean.util.io.ts.TransportStreamUtils.fillPAT;
import static com.kdy.bean.util.io.ts.TransportStreamUtils.fillPMT;
import static com.kdy.bean.util.io.ts.TransportStreamUtils.videoCodecToStreamType;
...
import com.kdy.bean.util.common.BufferUtils;
import com.kdy.bean.util.io.mp3.MP3BufferedDecoder;
import com.kdy.bean.util.io.mp3.MP3HeaderData;
import com.kdy.bean.util.io.ts.TSPacketFragment;
...
import com.kdy.bean.util.media.acc.AACFrame;
import com.kdy.bean.util.media.acc.AACUtils;
import com.kdy.bean.util.media.h264.H264CodecConfigParts;
import com.kdy.bean.util.media.h264.H264Utils;
 ...
 *
 */
public class MP4TOMPEGTSWriter {

	private static Logger log = LoggerFactory.getLogger(MP4TOMPEGTSWriter.class);
	
	private IMP4TOMPEGTSWriter writer;
	...
	public MP4TOMPEGTSWriter(IMP4TOMPEGTSWriter writer, IoBuffer videoConfig, IoBuffer audioConfig) {
		
		this.writer = writer;
		if(videoConfig != null) {
			videoCodec = (byte)TransportStreamUtils.getVideoCodec(videoConfig.get(0));
		...
		if(audioConfig != null) {
			audioCodec = (byte)TransportStreamUtils.getAudioCodec(audioConfig.get(0));
	...
	public void handleVideo(VideoData data) {
		...
		if (dataBuff.remaining() >= 2) {
			...
			int codec = TransportStreamUtils.getVideoCodec(firstByte);
			int frameType = TransportStreamUtils.getFrameType(firstByte);
			int paloadLen = 0;
			int naluNum = 0;
			videoCodec = codec;
			if ((codec == VideoCodec.AVC.getId()) && (secondByte != 1)) {
				...
				int naluLen;
				dataBuff.position(5);
				List<TSPacketFragment> tsPacketList = new LinkedList<TSPacketFragment>();
				...
				if (naluNum > 0) {
					...
					while (true) {
						...
						while (true) {
							...
							// once execute at the first time
							if (atf != 0) {
								int thirdByte = 3;
	...
	public void handleAudio(AudioData data) {
		...
		if (dataBuff.remaining() >= 2) {
			...
			int codecId = TransportStreamUtils.getAudioCodec(firstByte);
			audioCodec = codecId;
	...
	class WaitingAudio {
		...
		int size = 0;
		int codec = 0;
		List<TSPacketFragment> fragments = new LinkedList<TSPacketFragment>();
